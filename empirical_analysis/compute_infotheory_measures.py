...
output_data_dir = "/Users/abry4213/github/info_theory_visuals/empirical_analysis/infotheory_measures/"
...
def compute_info_theory_SPIs_for_subject(subject_ID, input_data_dir, basecalc, brain_region_lookup, source_base_region="lateraloccipital"):
    if os.path.isfile(f"{output_data_dir}/{subject_ID}_infotheory_measures.csv"):
        return 
    ...
    # Save to a CSV file
    this_subject_infotheory_results.to_csv(f"{output_data_dir}/{subject_ID}_infotheory_measures.csv", index=False)


subject_IDs = glob(f"{input_data_dir}/*.npy")
subject_IDs = [os.path.basename(x).split(".")[0] for x in subject_IDs]

n_jobs=8

for subject_ID in subject_IDs:
    compute_info_theory_SPIs_for_subject(subject_ID, input_data_dir, basecalc, brain_region_lookup, source_base_region=source_base_region)
