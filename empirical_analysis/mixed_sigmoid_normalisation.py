...
def mixed_sigmoid_func(x):
    # Compute IQR and median
    IQR = np.percentile(x, 75) - np.percentile(x, 25)
    median = np.median(x)
    # If IQR is zero, apply standard logistic transformation
    if IQR == 0:
        return 1/(1 + math.e**(- ( (x - np.mean(x)) / np.std(x) ) ))
    # Otherwise, apply mixed sigmoid transformation
    else:
        return 1/(1 + math.e**(- ( (x - median) / IQR/1.35 ) ))
